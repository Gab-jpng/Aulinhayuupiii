...
if (command === "update") {
  ...
  if (isNaN(index) || !newDescription) {
    console.error("Uso: update <índice> '<nova descrição>'");
    process.exit(1);
  }
  ...
  try {
    ...
    currentItem.updateDescription(newDescription);
    await todo.updateItem(index, currentItem);
...
if (command === "remove") {
  ...
  if (isNaN(index)) {
    console.error("Uso: remove <índice>");
    process.exit(1);
  }
